...
class UnknownListAdapter2(
    val context: Context,
    val arrayList: List<UnknownEntity2>,
    val productsListDao: UnknownListDao2
) :
    RecyclerView.Adapter<UnknownListAdapter2.ViewHolder>() {
    open class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var binding = CustomProductsBinding.bind(itemView)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return UnknownListAdapter2.ViewHolder(
    ...
    fun showDeleteAlertDialog(product: UnknownEntity2) {
    ...
    private fun updateDialog(item: UnknownEntity2) {
        ...
        btnSave.setOnClickListener {
            ...
            if (newName.isEmpty()) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    productsListDao.updateProduct(UnknownEntity2(item.id, newName))
                    (context as Activity).runOnUiThread {
                        dialog.dismiss()
